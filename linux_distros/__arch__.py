    ...
    for package in packages:
        ...
        try:
            if package_type in {"package", "AUR-package", "local-package"}:
                ...
                handle_flatpak_package(check_value, name, action, dry_run, package, hide)
...
def handle_standard_package(check_value, name, action, dry_run, package, hide):
    ...
    result = run(["pacman", "-Q"] + packages_to_check, stdout=PIPE, stderr=PIPE, check=True)
...
def package_installer(package, hide):
    ...
    try:
        if package_type == "package":
            run(["sudo", "pacman", "-S", install_value, "--noconfirm"], check=True, stderr=hide, stdout=hide)

        elif package_type == "local-package":
            local_package_installer(install_value, target_directory, hide)

        elif package_type == "service":
            run(["sudo", "systemctl", "restart", install_value], check=True, stderr=hide, stdout=hide)
            run(["sudo", "systemctl", "enable", install_value], check=True, stderr=hide, stdout=hide)

        elif package_type == "group":
            run(["sudo", "usermod", "-aG", install_value, current_user], check=True, stderr=hide, stdout=hide)

        elif package_type == "repo-flathub":
            run(["sudo", "flatpak", "remote-add", "--if-not-exists", "flathub", install_value], check=True, stderr=hide, stdout=hide)

        elif package_type == "package-flatpak":
            run(["sudo", "flatpak", "install", "-y", install_value], check=True, stderr=hide, stdout=hide)
...
def local_package_installer(install_value, target_directory, hide):
    run(["wget", "--progress=bar:force", "-O", "local.package.pkg.tar.zst", install_value], cwd=target_directory, check=True, stderr=hide, stdout=hide)
    run(["sudo", "pacman", "-U", "local.package.pkg.tar.zst", "--noconfirm"], cwd=target_directory, check=True, stderr=hide, stdout=hide)
    run(["sudo", "rm", "-f", "local.package.pkg.tar.zst"], cwd=target_directory, check=True, stderr=hide, stdout=hide)
...
def handle_aur_package(install_value, target_directory, hide):
    ...
    run(["makepkg", "-si", "--noconfirm"], cwd=repository_directory, check=True, stderr=hide, stdout=hide)
    run(["sudo", "rm", "-rf", install_value], cwd=target_directory, check=True, stderr=hide, stdout=hide)
...
def package_remover(package, hide):
    ...
    try:
        if package_type in {"package", "AUR-package", "local-package"}:
            run(["sudo", "pacman", "-R", remove_value, "--noconfirm"], check=True, stderr=hide, stdout=hide)
        
        elif package_type == "package-flatpak":
            run(["sudo", "flatpak", "remove", "-y", remove_value], check=True, stderr=hide, stdout=hide)
